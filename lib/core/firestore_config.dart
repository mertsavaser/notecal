...
void printFirestoreDebugInfo() {
  if (kDebugMode) {
    ...
    debugPrint(
        '[FirestoreConfig] Project ID: ${firestore.app.options.projectId}');
    debugPrint('[FirestoreConfig] Database: (default)');
    debugPrint('[FirestoreConfig] Host: ${firestore.settings.host}');
    debugPrint(
        '[FirestoreConfig] SSL Enabled: ${firestore.settings.sslEnabled}');
    debugPrint(
        '[FirestoreConfig] Emulator Enabled: ${isFirestoreEmulatorEnabled()}');
  }
}
